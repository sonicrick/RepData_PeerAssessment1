...
```{r }
library(plyr)
byday.act <- ddply(activity, .(date), summarise,
                     total.steps=sum(steps, na.rm=TRUE))
hist(byday.act$total.steps, main="Histogram of total steps per day",
        xlab="Steps", ylab="Days of occurences")
```
...
```{r }
byint.act <- ddply(activity, .(interval), summarise, average.steps=mean(steps, na.rm=TRUE))
with(byint.act, plot(interval, average.steps, type="l",
                     main="Average steps in each interval across all days",
...
```{r }
print(paste0("Interval with the highest average steps is ",
             with(byint.act, interval[which(average.steps==max(average.steps))])))
```
...
```{r }
byday.act.noNA <- ddply(act.noNA, .(date), summarise,
                     total.steps=sum(steps, na.rm=TRUE))
hist(byday.act.noNA$total.steps, main="Histogram of total steps per day",
        sub="(missing steps values imputed)",
        xlab="Steps", ylab="Days of occurences")
...
```{r }
act.noNA$weekpart <- factor(ifelse(weekdays(act.noNA$date) %in% c("Saturday", "Sunday"),"weekend", "weekday"))
```
...
```{r }
byint.act.noNA <- ddply(act.noNA, .(interval, weekpart), summarise,
...
```{r }
library(ggplot2)
qplot(interval, average.steps, data=byint.act.noNA, geom="line",
      main="Average steps in each interval across all days",
      sub="(missing steps values imputed)",
